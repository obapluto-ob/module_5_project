...
def save_job():
    ...
    db.get_or_404(Job, data["job_id"])

    if db.session.execute(select(SavedJob).filter_by(user_id=user_id, job_id=data["job_id"])).scalar_one_or_none():
...
def get_saved_jobs():
    user_id = int(get_jwt_identity())
    saved = db.session.execute(select(SavedJob).filter_by(user_id=user_id)).scalars().all()
...
def unsave_job(saved_id):
    saved = db.get_or_404(SavedJob, saved_id)
    user_id = int(get_jwt_identity())
